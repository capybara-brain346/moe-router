...
def plot_throughput(results, save_path="plots/throughput.png"):
    ...
    ax.plot(
        epochs, dense_throughput, "o-", label="Dense Model", linewidth=2, markersize=6
    )
...
def plot_validation_loss(results, save_path="plots/validation_loss.png"):
    ...
    ax.plot(
        eval_steps, dense_val_loss, "o-", label="Dense Model", linewidth=2, markersize=6
    )
    ax.plot(
        eval_steps, moe_val_loss, "s-", label="MoE Model", linewidth=2, markersize=6
    )
    ...
    improvement = (
        ((final_dense - final_moe) / final_dense * 100) if final_dense > 0 else 0
    )
...
def plot_expert_usage(results, save_path="plots/expert_usage.png"):
    ...
    for idx, (layer_idx, usage) in enumerate(
        sorted(expert_usage.items(), key=lambda x: int(x[0]))
    ):
        ax = axes[idx]
        ...
        bars = ax.bar(
            expert_ids, percentages, color="steelblue", edgecolor="black", linewidth=1.2
        )
...
def plot_combined_summary(results, save_path="plots/summary.png"):
    ...
    ax1.plot(
        eval_steps, moe_val_loss, "s-", label="MoE Model", linewidth=2.5, markersize=7
    )
    ...
    if expert_usage:
        for idx, (layer_idx, usage) in enumerate(
            ...
            ax.set_title(
                f"Layer {layer_idx} Expert Usage", fontsize=12, fontweight="bold"
            )
            ax.set_xticks(expert_ids)
            ax.legend(fontsize=9)
            ax.grid(True, alpha=0.3, axis="y")
...
Final Val Loss:
  ...
  MoE: {moe_val_loss[-1]:.4f}
    ...
    fig.suptitle(
        "Phase 2: Dense vs MoE Transformer Comparison",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
...
def generate_report(results):
    ...
    print(
        f"  Parameter Ratio (MoE/Dense): {config['moe_params'] / config['dense_params']:.2f}x"
    )
    ...
    for layer_idx, usage in sorted(expert_usage.items(), key=lambda x: int(x[0])):
        ...
        print(
            f"    Layer {layer_idx}: {balance_score:.1f}% balanced (std={std_dev:.2f}%)"
        )
